...
<body>
    ...
    <?php
        ...
        <?php
            ...
            //selects table named customer from database and saves it to $sql variable
            $sql = "SELECT id,  transaction_amount, balance, DATE_FORMAT(date_time, '%Y-%m-%d') DATEONLY, DATE_FORMAT(date_time,'%H:%i:%s') TIMEONLY FROM shopkeeper ORDER BY id DESC LIMIT $name"; //++++++++++++++++( actually used for transaction showing)++++++++++
            //runs mathi ko $sql
            $result = $conn->query($sql);
